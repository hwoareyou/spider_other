...
def request(url,category_id):

    print('采集商品：',url)
    if category_id:
        referer = 'http://www.wittysafe.com/s/index.php/category?cid=' + str(category_id)
    else:
        referer = 'http://www.wittysafe.com/s/index.php/category'
    headers = {
        "Host": "www.wittysafe.com",
...
def parse(html_source,sellItemId):
    ...
    title = str(html.xpath('string(/html/head/title)')).strip().replace('【官方店】','')
    product_info['title'] = title

    introduction = str(html.xpath('string(//div[@class="detail-profile"])')).strip()
    product_info['introduction'] = introduction

    promotion_price = str(html.xpath('string(//*[@id="payForm"]/div[1]/div[2]/div/span[1])')).strip() #  促销价格
    price = str(html.xpath('string(//*[@id="payForm"]/div[1]/div[2]/div/span[2])')).strip() # 原价
    product_info['promotion_price'] = promotion_price
    product_info['price'] = price

    pic = str(html.xpath('//*[@id="payForm"]/div[1]/div[1]/img/@src')[0])
    product_info['pic'] = pic

    content = html.xpath('//div[@class="detail-block"]//img')
    content_pics = [dict(item.attrib)['src'] for item in content]
...
def get_product_by_category(category_id):
    ...
    if category_id:
        ...
        post_data = {}
    url = 'http://www.wittysafe.com/s/index.php/category'
    res = requests.post(url,headers=headers,data=post_data)
    html = etree.HTML(res.text)
    products_link = [dict(item.attrib)['href'] for item in html.xpath('//div[@id="menu_con"]//a')]
...
def save(product_info, sellItemId, category_name):
    ...
    if not res:
        ...
        value = [(0,product_info['title'], category_id, 1, product_info['marketPrice'], product_info['price'],
                  ...
                  description, 1, 1, str(picture), 3, 1)]
...
def main():
    ...
    for category_id in range(13):
        ...
        products_link = get_product_by_category(category_id)
        for product_link in products_link:
            sellItemId = re.search(r'index.php/p/(.+)',product_link).group(1)
            html = request(product_link,category_id)
